...
void
FontAssetLoadList::ui_edit_props(
  ...
  const entt::registry& registry)
{
  ...
  if ( ImGui::CollapsingHeader("Fonts to load", ImGuiTreeNodeFlags_DefaultOpen) == false )
    return;
  ...
  if ( ImGui::BeginPopup("##fontAddPopup") )
  {
    ...
    for ( const auto& fontId : fontList )
    {
      ...
      if ( std::find(fontsToLoad.begin(), fontsToLoad.end(),
                     fontId) != fontsToLoad.end() )
        continue;
  ...
  if ( ImGui::BeginTable( "FontsToLoadList", 1, tableFlags) == false )
    return;

  ImGui::TableNextColumn();

  for ( auto iter = fontsToLoad.begin();
        iter != fontsToLoad.end();
        ++iter )
  {
    ImGui::PushID(std::distance(fontsToLoad.begin(), iter));

    if ( ImGui::SmallButton("-##fontDel") )
      iter = fontsToLoad.erase(iter);

    if ( iter == fontsToLoad.end() )
    {
      ImGui::PopID();
      break;
    }
  ...
  ImGui::EndTable(); // FontsToLoadList
}
